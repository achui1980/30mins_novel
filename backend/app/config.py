...
# Seconds before a Bedrock invoke_model call gives up. Without this, a hung
# endpoint could pin a worker thread indefinitely (boto3's default read
# timeout is 60s per attempt but botocore retries can multiply that).
# Mirrors NOVEL_KG_OPENAI_COMPATIBLE_TIMEOUT's default for consistency.
BEDROCK_TIMEOUT = float(_env("NOVEL_KG_BEDROCK_TIMEOUT", "120"))
...
# --- Cross-request job concurrency -----------------------------------------
# GLOBAL_EXTRACT_CONCURRENCY (above) only bounds LLM calls *within* a single
# job. Without a separate cap here, every POST /works dispatches its pipeline
# immediately via BackgroundTasks with no cross-request limit, so N
# concurrent uploads each try to run GLOBAL_EXTRACT_CONCURRENCY LLM calls at
# once. This caps how many jobs may run their pipeline concurrently; extra
...
# work_id is generated by store.new_work_id() as a 12-char lowercase hex
# string, but some existing tests/fixtures use hand-written ids containing
# letters/digits/underscore/hyphen. The one thing that must never be allowed
# through is path-traversal metacharacters (".", "/", "\\", null bytes, ...),
# since work_id is interpolated directly into a filesystem path below. This
# pattern is intentionally permissive on charset (not hex-only) to preserve
# backward compatibility with existing callers/tests, while still rejecting
# anything that could escape DATA_ROOT.
_WORK_ID_RE = re.compile(r"^[A-Za-z0-9_-]{1,64}$")


def _validate_work_id(work_id: str) -> None:
    if not isinstance(work_id, str) or not _WORK_ID_RE.match(work_id):
        raise InvalidWorkIdError(f"invalid work_id: {work_id!r}")
