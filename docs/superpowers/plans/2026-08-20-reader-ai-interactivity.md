...
**Final Verification status (completed):** All 3 steps passed. Backend suite: 206 passed, 0 failed. Frontend build: clean. Manual smoke test: passed on both a fresh work and an old (`白夜行`, pre-dates this feature) work, with one bug found and fixed along the way — see below.

**Post-verification fix (commit `994ba82`):** The manual smoke test surfaced a race between Task 14c's reading-position restore effect and Task 14e's cross-tab jump effect in `RawTextTab.jsx` — when a work had a saved reading position AND a "查看原文→" jump was triggered on the same mount, the restore effect's scroll silently won and discarded the jump with no visible feedback. Fixed by having the restore effect bail out when a pending, unhandled `jump` prop is present (deferring entirely to the jump effect). Empirically verified via production build (`vite build` + `vite preview`): regression check (no pending jump — restore still works), the fix itself (jump now correctly wins over a saved position), and no reverse-direction race (the progress-tracking observer doesn't fight the jump's scroll afterward). Commit: `"fix: prevent reading-progress restore from overriding pending cross-tab jump in RawTextTab"`.

---

## Known Issues (deferred, not fixed as part of this plan)

Two pre-existing bugs were discovered during this implementation's review cycles. Both are **independent of this feature** (reproducible on code paths this plan didn't touch, or triggered by pre-existing patterns this plan didn't introduce) and were deliberately left unfixed as out of scope. Documented here so they aren't lost as tribal knowledge:

1. **`loadChapter` batching race** (`frontend/src/components/tabs/RawTextTab.jsx`, the `loadChapter`/`shouldFetch`-guard function and its lazy-load `IntersectionObserver` effect, introduced in Task 14a). When multiple chapters' `IntersectionObserver` entries batch into a single React state update, the `shouldFetch` closure variable can be read before React flushes the batched updater from a sibling entry in the same batch, causing one or more non-first-visible chapters to get stuck permanently at `loading:true` ("加载中…", never fetched). Reproducible on short novels (can affect *all* chapters) and longer ones (affects some non-first chapters). Suggested fix direction: read live state via a ref or restructure the guard to not rely on a single closure variable across a batch of near-simultaneous calls.

2. **`AskTab.jsx` infinite-render-loop** ("Maximum update depth exceeded", triggers a 500+ duplicate-request storm to `/graph`, `/timeline`, `/chapters/{id}/text` while the Ask tab is open). Root cause: `ReaderPage.jsx`'s `askAbout`, `askAboutSelection`, and the fallback `questions` array are recreated as new references on every `ReaderPage` render; `AskTab.jsx`'s `useEffect(() => setRight(<SuggestedQuestions .../>), [questions, onAsk, setRight])` therefore fires on every render and calls `setRight` (a parent state setter), which triggers another `ReaderPage` render, closing the loop. Confirmed isolated to `AskTab` only (`TimelineTab`/`CharactersTab`/`GraphTab`'s own `setRight` effects don't depend on `onViewChapter`, so they don't loop). Suggested fix: wrap `askAbout`/`askAboutSelection` in `useCallback` in `ReaderPage.jsx`, and/or fix `AskTab.jsx`'s effect dependency array to not depend on a freshly-created `onAsk` reference every render.
